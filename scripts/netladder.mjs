...
const seed = Number(flag('seed', 8080808));
...
console.log(`net ladder: ${games} games at ${iterations} simulations, ${bounds.length} processes`);
...
function runShard([from, to]) {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [cli, 'src/ai/nn/netladder-shard.ts'], {
      env: { ...process.env, NL_FROM: String(from), NL_TO: String(to), NL_ITERATIONS: String(iterations), NL_SEED: String(seed) },
