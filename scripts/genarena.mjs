...
console.log(`generation head-to-head: ${games} games at ${iterations} simulations, ${bounds.length} processes`);
...
function runShard([from, to]) {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [cli, 'src/ai/nn/gen-arena-shard.ts'], {
      env: { ...process.env, GA_FROM: String(from), GA_TO: String(to), GA_ITERATIONS: String(iterations), GA_SEED: String(seed) },
