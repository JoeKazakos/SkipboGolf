...
console.log(`belief arena: ${games} games at ${iterations} simulations, ${bounds.length} processes`);
...
function runShard([from, to]) {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [cli, 'src/ai/nn/belief-arena-shard.ts'], {
      env: { ...process.env, BA_FROM: String(from), BA_TO: String(to), BA_ITERATIONS: String(iterations), BA_SEED: String(seed) },
